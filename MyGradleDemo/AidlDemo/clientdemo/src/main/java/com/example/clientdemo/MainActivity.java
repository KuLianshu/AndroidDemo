...
public class MainActivity extends AppCompatActivity {
    ...
    public void borrowBook(View view) {
        checkConnected();
        try {
            int resultCode = iMyAidlInterface.borrowBook("百年孤独");
    ...
    public void returnBook(View view) {
        checkConnected();
        try {
            int resultCode = iMyAidlInterface.returnBook("百年孤独");
            ...
            iMyAidlInterface.getBooks(new ISearchBooksCallback.Stub() {
                ...
                public void onSuccess(final List<Book> books) throws RemoteException {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            LogUtil.d(TAG, "客户端收到回调了 onSuccess--数量：" + books.size());
    ...
    public void borrowOtherBook(View view) {
        checkConnected();
        try {
            Book book = new Book("霍乱时期的爱情", 1);
            int resultCode = iMyAidlInterface.borrowOtherBook(book);
            //打印被服务器改变的值
            LogUtil.d(TAG,"借到的图书为："+book.getName());
